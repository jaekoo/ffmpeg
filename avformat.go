...
func (f *OutputFormat) VideoCodec() uint32 {
	coutput := *(*C.struct_AVOutputFormat)(unsafe.Pointer(f))

	return coutput.video_codec
}

func (f *OutputFormat) Flags() int32 {
	coutput := *(*C.struct_AVOutputFormat)(unsafe.Pointer(f))

	return int32(coutput.flags)
}

func (f *OutputFormat) SetVideoCodec(codec uint32) {
	coutput := (*C.struct_AVOutputFormat)(unsafe.Pointer(f))
	coutput.video_codec = codec
...
func AvformatGetClass() *Class {
	return (*Class)(unsafe.Pointer(C.avformat_get_class()))
}
...
func AvformatOpenInput(ps **FormatContext, fi string, fmt *InputFormat, d **Dictionary) int {
...
func AvIOOpen(pb *AvIOContext, url string, flags int) (err error) {
	urlStr := C.CString(url)
	defer C.free(unsafe.Pointer(urlStr))

	err = ErrorFromCode(int(C.avio_open((**C.AVIOContext)(unsafe.Pointer(&pb)), urlStr, C.int(flags))))
